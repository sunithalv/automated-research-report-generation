...
class ReportService:
    ...
    def start_report_generation(self, topic: str, max_analysts: int):
        ...
        try:
            ...
            #Get analyst details to display in UI
            state = self.graph.get_state(thread)
            analysts = state.values.get("analysts")

            return {"thread_id": thread_id,"analysts":analysts, "message": "Pipeline initiated successfully."}
    ...
    def submit_feedback(self, thread_id: str, feedback: str):
        ...
        try:
            ...
            if feedback not in (None, ""):
                self.logger.info("Processing feedback", feedback=feedback)
                #Get analyst details to display in UI
                state = self.graph.get_state(thread)
                analysts = state.values.get("analysts")
            return {"message": "Feedback processed successfully","thread_id": thread_id,"analysts":analysts}
        except Exception as e:
